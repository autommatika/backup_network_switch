...
if [ ! -e '/tmp/inet' ];then
    if [[ $( ping -c 1 -q 12.34.56.78 | grep -oP '\d+(?=% packet loss)') == 0 ]];then
...
if [[ $(ping -c 1 -q 12.34.56.78 | grep -oP '\d+(?=% packet loss)') == 100 ]];then
...
elif [[ $(ping -c 1 -q 12.34.56.78 | grep -oP '\d+(?=% packet loss)') == 0 ]];then
